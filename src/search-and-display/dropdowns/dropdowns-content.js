...
import { capitalizeFirstLetter, fillDropdownOptions } from './dropdowns-utils.js';
...
function initializeDropdownOptions(recipes) {
  ...
  recipes.forEach(recipe => {
    recipe.ingredients.forEach(ing => {
      ...
      allItems.ingredient.add(capitalizeFirstLetter(String(ingredient.ingredient)));
    });
    ...
    if (appliance.appliance && typeof appliance.appliance === 'string') {
      allItems.appliance.add(capitalizeFirstLetter(String(appliance.appliance)));
    }

    recipe.ustensils.forEach(ust => {
      const ustensil = new CookFactory(ust, 'ustensile');
      if (typeof ustensil[0].ustensile === 'string') {
        allItems.ustensile.add(capitalizeFirstLetter(String(ustensil[0].ustensile)));
      }
    });
  });
